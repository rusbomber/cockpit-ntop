...
async function updateServiceSwitch() {
	...
	if (stubMode()) {
		...
		clusterActive.value = await isServiceActive(serviceName, props.name);
		clusterEnabled.value = await isServiceEnabled(serviceName, props.name);
	}

	if (clusterEnabled.value) {
		clusterSwitch.value = true;
	}
